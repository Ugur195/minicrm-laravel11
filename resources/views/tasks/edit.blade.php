                ...
                <form method="POST" action="{{ route('tasks.update', $task) }}">
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="deadline_at" class="block mt-1 w-full" type="date" name="deadline_at"
                                      :value="old('deadline_at', $task->deadline_at)" required/>
                    ...
                    <div>
                        ...
                        <select id="client_id" name="client_id" class="block mt-1 w-full" required>
                            @foreach($clients as $client)
                                <option
                                    value="{{ $client->id }}" @selected(old('client_id', $task->client_id) == $client->id)>
                                    {{ $client->company_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="project_id" name="project_id" class="block mt-1 w-full" required>
                            @foreach($projects as $project)
                                <option
                                    value="{{ $project->id }}" @selected(old('project_id', $task->project_id) == $project->id)>
                                    {{ $project->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="block mt-1 w-full" required>
                            @foreach(\App\Enums\TaskStatus::cases() as $status)
                                <option
                                    value="{{ $status->value }}" @selected(old('status', $task->status->value) == $status->value)>
                                    {{ $status->value }}
                                </option>
                            @endforeach
                        </select>
